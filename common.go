...
type Config struct {
	...
	// ImpersonateCert is the raw DER bytes of the real TLS certificate of the
	// server being impersonated (e.g. fetched from the Dest host). When set,
	// per-connection certificates mirror its Subject, validity period, SANs,
	// and key-usage fields exactly, making the cert indistinguishable from the
	// real one to passive DPI. Only the public key and signature differ.
	ImpersonateCert []byte
