...
var (
	// ed25519Priv and signedCertMldsa65 are used only by the ML-DSA-65 path,
	// which requires a fixed cert structure with a 3309-byte extension at
	// a known offset (cert[126:]). The standard path generates certs per-connection.
...
func init() {
	...
	// Build the mldsa65 template cert with realistic fields.
	var serial [16]byte
	io.ReadFull(rand.Reader, serial[:])
	serial[0] |= 0x01 // ensure non-zero
	var jitter [2]byte
	io.ReadFull(rand.Reader, jitter[:])
	notBefore := time.Now().AddDate(0, 0, -(30 + int(jitter[0])%60)).UTC().Truncate(time.Second)
	notAfter := notBefore.AddDate(0, 0, 365+int(jitter[1])%365)

	certificateMldsa65 := x509.Certificate{
		SerialNumber:    new(big.Int).SetBytes(serial[:]),
		NotBefore:       notBefore,
		NotAfter:        notAfter,
		KeyUsage:        x509.KeyUsageDigitalSignature,
		ExtraExtensions: []pkix.Extension{{Id: []int{0, 0}, Value: empty[:3309]}},
	}
...
// buildConnCert generates a fresh ed25519 certificate for a single connection.
//
// If impersonateCert is non-nil (raw DER of a real server cert), the generated
// cert mirrors its Subject, validity period, SANs, and key-usage fields exactly.
// Otherwise, a plausible-looking fallback cert is built using serverName as CN.
//
// In both cases a fresh ed25519 key pair is generated and the real signature is
// overwritten with HMAC-SHA512(authKey, pubKey) for REALITY authentication.
func buildConnCert(authKey []byte, serverName string, impersonateCert []byte) (certDER []byte, priv ed25519.PrivateKey, err error) {
	...
	if len(impersonateCert) > 0 {
		real, parseErr := x509.ParseCertificate(impersonateCert)
		if parseErr != nil {
			err = fmt.Errorf("reality: buildConnCert: parse ImpersonateCert: %w", parseErr)
			return
		}
		template = &x509.Certificate{
			...
			ExtKeyUsage:    real.ExtKeyUsage,
			DNSNames:       real.DNSNames,
			IPAddresses:    real.IPAddresses,
			EmailAddresses: real.EmailAddresses,
		}
	}
	...
	if template == nil {
		...
		var jitter [2]byte
		io.ReadFull(rand.Reader, jitter[:]) //nolint:errcheck
		daysAgo := 30 + int(jitter[0])%60
		daysValid := 365 + int(jitter[1])%365
		notBefore := time.Now().AddDate(0, 0, -daysAgo).UTC().Truncate(time.Second)
	...
	certDER, err = x509.CreateCertificate(rand.Reader, template, template, ed25519.PublicKey(priv[32:]), priv)
	if err != nil {
		return
	}

	// Replace the ed25519 signature (last 64 bytes of DER) with
	// HMAC-SHA512(authKey, pubKey). The REALITY client verifies this HMAC
	// instead of the standard X.509 signature chain.
	h := hmac.New(sha512.New, authKey)
	h.Write(priv[32:])
	h.Sum(certDER[:len(certDER)-64])
	return
}
	...
	*/
	{
		...
		if len(c.config.Mldsa65Key) > 0 {
			// ML-DSA-65 path: cert has a 3309-byte extension at a fixed offset (cert[126:])
			// so we must use the globally pre-built cert and overwrite its signature.
			privKey = ed25519Priv
			certDER = bytes.Clone(signedCertMldsa65)
			...
			mldsa65.SignTo(key.(*mldsa65.PrivateKey), h.Sum(nil), nil, false, certDER[126:]) // fixed location
		} else {
			// Standard path: generate a fresh ed25519 cert per connection.
			var err error
			certDER, privKey, err = buildConnCert(c.AuthKey, c.serverName, c.config.ImpersonateCert)
			if err != nil {
				return err
			}
		}
